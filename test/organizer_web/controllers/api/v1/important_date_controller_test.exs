defmodule OrganizerWeb.API.V1.ImportantDateControllerTest do
  use OrganizerWeb.ConnCase

  import Organizer.AccountsFixtures

  describe "unauthenticated access" do
    test "returns 401 on list", %{conn: conn} do
      conn = get(conn, ~p"/api/v1/important-dates")

      assert conn.status == 401

      assert json_response(conn, 401) == %{
               "error" => %{"code" => "unauthorized", "message" => "authentication required"}
             }
    end
  end

  describe "authenticated access" do
    setup :register_and_log_in_user

    test "creates and lists important dates", %{conn: conn} do
      conn =
        post(conn, ~p"/api/v1/important-dates", %{
          "important_date" => %{
            "title" => "Renovacao passaporte",
            "category" => "personal",
            "date" => Date.to_iso8601(Date.add(Date.utc_today(), 10))
          }
        })

      assert %{"data" => %{"id" => id, "title" => "Renovacao passaporte"}} =
               json_response(conn, 201)

      conn = get(recycle(conn), ~p"/api/v1/important-dates")
      assert %{"data" => dates} = json_response(conn, 200)
      assert Enum.any?(dates, &(&1["id"] == id))
    end

    test "returns validation error when payload is invalid", %{conn: conn} do
      conn =
        post(conn, ~p"/api/v1/important-dates", %{
          "important_date" => %{
            "title" => "x",
            "category" => "unknown",
            "date" => Date.to_iso8601(Date.utc_today())
          }
        })

      assert %{"error" => %{"code" => "validation_error", "details" => details}} =
               json_response(conn, 422)

      assert Map.has_key?(details, "title")
      assert Map.has_key?(details, "category")
    end

    test "enforces user data isolation for show update and delete", %{conn: conn} do
      conn =
        post(conn, ~p"/api/v1/important-dates", %{
          "important_date" => %{
            "title" => "Data privada",
            "category" => "work",
            "date" => Date.to_iso8601(Date.add(Date.utc_today(), 3))
          }
        })

      %{"data" => %{"id" => id}} = json_response(conn, 201)

      other_user_conn =
        build_conn()
        |> log_in_user(user_fixture())

      show_conn = get(other_user_conn, ~p"/api/v1/important-dates/#{id}")
      assert json_response(show_conn, 404)["error"]["code"] == "not_found"

      update_conn =
        put(other_user_conn, ~p"/api/v1/important-dates/#{id}", %{
          "important_date" => %{
            "title" => "invadir",
            "category" => "personal",
            "date" => Date.to_iso8601(Date.utc_today())
          }
        })

      assert json_response(update_conn, 404)["error"]["code"] == "not_found"

      delete_conn = delete(other_user_conn, ~p"/api/v1/important-dates/#{id}")
      assert json_response(delete_conn, 404)["error"]["code"] == "not_found"
    end
  end
end
